...
ProgramData is a hidden folder. You may have to enable the option to show hidden folders by selecting the drop-down 'View' menu in File Explorer, hovering over 'Show' and selecting 'Hidden items' (See 'hiddenitems.png').
Once the file is in the correct folder, simply run the 'sfils.sql' script, also in the 'scripts' folder, on your server. As long as your current account has the correct permissions, it will create a 'sfils' schema with a 'patrons' table, as well as three other tables that contain additional information.
...
After that, you should be able to run the program in the command prompt as you would any Python program:
	> python3 sfils.py
The program will ask you to log in to your server. Remember to make sure the server is running, and to use an account that has the proper permissions for editing.
The program can perform three simple operations on the database:
